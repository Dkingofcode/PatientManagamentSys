...
function PaymentMethodSelection({ data, updateData, onComplete, onBack, }) {
    // const { user } = useAuth();
    ...
    return (<div className="bg-white rounded-lg shadow-sm p-8">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={onBack} className="flex items-center space-x-2 px-4 py-2 text-gray-600 hover:text-gray-800 hover:bg-gray-100 rounded-lg transition-colors">
            <ArrowLeft size={16}/>
            <span>Back</span>
          </button>
        </div>
        <p className="text-gray-600">
          Select your preferred payment method and enter the payment details
        </p>
      </div>
        ...
        <div className="space-y-3">
          {data.tests?.map((test, index) => (<div key={index} className="flex justify-between items-center">
              ...
              <div className="flex items-center space-x-2">
                {test.price && (<span className="line-through text-gray-400 text-sm">
                    ₦{test.price}
                  </span>)}
          ...
          {discountAmount > 0 && (<div className="flex justify-between items-center text-green-600 border-t pt-2">
              <span>
                Discount ({discountPercent}% - {data.category?.toUpperCase()})
              </span>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6 mb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {paymentOptions.map((option) => {
            ...
            return (<button key={option.id} onClick={() => handlePaymentMethodSelect(option.id)} className={`p-6 rounded-lg border-2 transition-all ${isSelected
                    ? `${option.borderColor} ${option.bgColor}`
                    : "border-gray-200 hover:border-gray-300 hover:bg-gray-50"}`}>
                <div className="flex flex-col items-center">
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-3 ${isSelected ? option.color : "bg-gray-100"}`}>
        ...
        {selectedPaymentMethod && selectedPaymentMethod !== "credit" && (<div className="space-y-4 mb-6">
                ...
                return (<div key={methodId} className="flex items-center space-x-4">
                  ...
                  <div className="flex-1">
                    <div className="relative">
                      ...
                      <input type="number" min="0" max={totalAmount} step="0.01" value={paymentAmounts[methodId] || ""} onChange={(e) => handleAmountChange(methodId, parseFloat(e.target.value) || 0)} className="w-full pl-8 pr-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="0.00"/>
                    </div>
                  </div>
                </div>);
            })}
        ...
        {/* Credit Amount Input */}
        {selectedPaymentMethod === "credit" && (<div className="mt-4 p-4 bg-orange-50 rounded-lg border border-orange-200 mb-6">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-medium text-orange-800">
                Credit Amount
              </span>
              <div className="relative">
                <span className="absolute left-3 top-1/2 transform -translate-y-1/2 text-orange-600">
                  ₦
                </span>
                <input type="number" min="0" max={totalAmount} step="0.01" value={creditAmount} onChange={(e) => setCreditAmount(parseFloat(e.target.value) || 0)} className="w-32 pl-8 pr-3 py-2 border border-orange-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500" placeholder="0.00"/>
              </div>
            </div>
      ...
      {selectedPaymentMethod && (<div className="bg-white border border-gray-200 rounded-lg p-6 mb-8">
      ...
      {selectedPaymentMethod && (<div className="bg-blue-50 rounded-lg p-4 mb-8">
      ...
      <div className="flex justify-end">
        <button onClick={handleComplete} disabled={!selectedPaymentMethod || !isPaymentComplete()} className="px-8 py-3 bg-purple-700 text-white rounded-lg hover:bg-purple-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors font-medium">
