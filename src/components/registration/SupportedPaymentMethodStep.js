...
import { CreditCard, Banknote, ArrowLeftRight, Calculator, DollarSign, Receipt, } from "lucide-react";
function SupportedPaymentMethodsStep({ data, updateData, onNext, onPrev, }) {
    ...
    // };
    const subtotal = data.tests?.reduce((sum, test) => test.price + sum, 0) || 0;
    ...
    // Update data whenever payment details change
    useEffect(() => {
    ...
    }, [selectedPaymentMethod, paymentAmounts, creditAmount]);
    return (<div>
      <div className="mb-6">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6 mb-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {paymentOptions.map((option) => {
            ...
            return (<button key={option.id} onClick={() => handlePaymentMethodSelect(option.id)} className={`p-4 rounded-lg border-2 transition-all ${isSelected
      ...
      {selectedPaymentMethod && (<div className="bg-white border border-gray-200 rounded-lg p-6 mb-6">
      ...
      {selectedPaymentMethod && (<div className="bg-blue-50 rounded-lg p-4 mb-6">
      ...
      <div className="flex justify-between">
        ...
        <button onClick={handleNext} disabled={!selectedPaymentMethod || !isPaymentComplete()} className="px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
          Next: Complete Payment
        </button>
      </div>
    </div>);
}
export default SupportedPaymentMethodsStep;
