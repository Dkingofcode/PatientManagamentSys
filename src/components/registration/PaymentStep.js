...
function PaymentStep({ data, updateData, onPrev, onComplete, }) {
    const { user } = useAuth();
    ...
    const paymentData = data.paymentData || initialPaymentData;
    const { selectedPaymentMethod = "", paymentMethods = [], paymentAmounts = {}, creditAmount = 0, totalAmount = 0,
    // totalPaid = 0,
    // remainingBalance = 0
     } = paymentData;
    ...
    return (<div>
      ...
      <div className="bg-gray-50 rounded-lg p-6 mb-6">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {data.tests?.map((test, index) => (<div key={index} className="flex justify-between items-center text-sm">
                <span className="text-gray-700">{test.name}</span>
                <div className="flex items-center space-x-2">
                  {getCurrentTestPrice(test) !== test.prices["walk-in"] && (<span className="line-through text-gray-400">
                      ${test.prices["walk-in"]}
                    </span>)}
                  <span className="font-medium">
                    ${getCurrentTestPrice(test)}
                  </span>
                </div>
              </div>))}
          </div>
      ...
      <div className="flex justify-between">
        ...
        <button onClick={handleComplete} disabled={!isPaymentComplete} className="px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
