...
import { Upload, FileImage, Search, Calendar, User, Download, Eye, Trash2, } from "lucide-react";
function XRayDashboard() {
    ...
    <div className="max-w-7xl mx-auto space-y-8">
      ...
      <div className="bg-white rounded-2xl shadow-lg p-8">
        ...
        <div className={`border-2 border-dashed rounded-xl p-12 text-center transition-colors ${dragActive
          ...
          <button onClick={() => fileInputRef.current?.click()} className="px-8 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
            Choose Files
          </button>
          <input ref={fileInputRef} type="file" multiple accept="image/*,.dcm" onChange={handleFileInput} className="hidden"/>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-lg p-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
          {categories.map(category => (<div key={category} className={`p-6 rounded-xl border-2 cursor-pointer transition-colors ${selectedCategory === category
                ? 'border-blue-500 bg-blue-50'
                : 'border-gray-200 hover:border-gray-300'}`} onClick={() => setSelectedCategory(category)}>
      ...
      <div className="bg-white rounded-2xl shadow-lg">
        <div className="p-8 border-b border-gray-200">
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex space-x-3">
              <button onClick={() => setSelectedCategory('all')} className={`px-4 py-2 rounded-lg text-sm font-medium ${selectedCategory === 'all'
          ...
          <div className="relative">
            ...
            <input type="text" placeholder="Search by patient name or filename..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-12 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"/>
          </div>
        </div>
        ...
        <div className="p-8">
          <div className="space-y-4">
            ...
            const patient = patients.find(p => p.id === file.patientId);
            return (<div key={file.id} className="flex items-center justify-between p-6 border border-gray-200 rounded-xl hover:bg-gray-50 transition">
