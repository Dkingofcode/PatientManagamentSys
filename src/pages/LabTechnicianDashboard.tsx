...
import { useNotifications } from "../contexts/NotificationContext"; // Assumed notification context
...
function LabTechnicianDashboard() {
  ...
  const { addNotification } = useNotifications(); // Notification context for Doctor Dashboard
...
interface ResultsEntryModalProps {
  testId: string | null;
  onClose: () => void;
  onSubmit: (results: { data: string; notes: string; file?: File }) => void;
}
...
function ResultsEntryModal({ testId, onClose, onSubmit }: ResultsEntryModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (resultType === 'upload' && file) {
      ...
      formData.append('resultText', results);
      formData.append('status', abnormalFlag ? 'abnormal' : 'normal');
    }
    formData.append('techNotes', techNotes);
    ...
    try {
      ...
      if (response.ok) {
        onSubmit({ data: results, notes: techNotes, file: file || undefined });
        onClose();
      } else {
