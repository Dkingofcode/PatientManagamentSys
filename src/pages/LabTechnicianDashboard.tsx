...
const dummyAssignmentTestsProduct: Appointment[] = [
  {
    id: "1",
    patientId: "p1",
...
function LabTechnicianDashboard() {
  ...
  const calculateAverageTAT = () => {
    const completedTests = filteredTests.filter(
      (t) => t.completionTime && t.startTime
    );
  ...
  const calculateErrorRate = () => {
    const qcFailed = filteredTests.filter(
      (t) => t.qcStatus === "failed"
    ).length;
    const totalCompleted = filteredTests.filter(
      (t) => t.status === "lab-completed" || t.status === "completed"
    ).length;
  ...
  const performanceMetrics = {
    testsCompleted: filteredTests.filter(
      (t) => t.status === "lab-completed" || t.status === "completed"
    ).length,
  ...
  const stats = [
    {
      ...
      value: filteredTests.filter(
        (t) => t.status === "completed" && t.qcStatus === "passed"
      ).length,
      icon: CheckCircle,
      color: "bg-gray-700",
    },
      ...
      filteredTests.forEach((test) => {
        if (test.qcStatus === "pending") {
          updateAppointment(test.id, {
            qcStatus:
              qcDataUpdate.internal?.status === "passed" ? "passed" : "failed",
          });
        }
      });
    }
  ...
  return (
    <Layout title="">
      <div className="space-y-6 px-2 sm:px-4">
                  ...
                  <div className="bg-white rounded-lg border border-gray-200 p-4 sm:p-6">
                    ...
                    <div className="space-y-2 sm:space-y-3">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span
                          className={`px-2 py-1 text-xs font-medium rounded-full ${
                            qcData.calibration.status === "valid"
                              ? "bg-green-100 text-green-800"
                              : "bg-red-100 text-red-800"
                          }`}
                        >
                        ...
                        .map((test, i) => (
                          ...
                          >
                            ...
                            <p className="text-xs text-gray-600">
                              Completed:{" "}
                              {new Date(test.completionTime!).toLocaleString(
                                "en-NG",
                                {
                                  timeZone: "Africa/Lagos",
                                }
          ...
          <ResultsEntryModal
            ...
            onSubmit={(results) => {
              if (selectedTest) {
                ...
                // Notify Doctor Dashboard of new results
                const appointment = testAssignments.find(t => t.id === selectedTest);
                if (appointment) {
                  addNotification({
                    title: 'New Test Results Available',
                    message: `Results for patient ${appointment.patientName} (Appointment ID: ${appointment.id}) are ready for review.`,
                    type: 'info',
                    category: 'appointment',
                    priority: 'high',
                    recipientRole: 'doctor', // Targets doctors
                    // timestamp removed to match Notification type
                  });
                }
              }
              setShowResultsModal(false);
        ...
        {showUploadReportModal && selectedAppointment && (
          <UploadReportModal
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4 sm:space-y-6">
          ...
          {resultType === "manual" ? (
            ...
            <div>
              ...
              />
              {uploadStatus && (
                <p className="text-red-600 text-sm mt-2">{uploadStatus}</p>
              )}
              {file && (
...
function QCLoggingModal({
  currentData,
  onClose,
  onSubmit,
}: QCLoggingModalProps) {
  const [internalStatus, setInternalStatus] = useState(
    currentData.internal.status
  );
  const [externalStatus, setExternalStatus] = useState(
    currentData.external.status
  );
  const [calibrationStatus, setCalibrationStatus] = useState(
    currentData.calibration.status
  );
            ...
            {instruments.map((inst, index) => (
              <div key={index} className="flex items-center space-x-2 mb-2">
                ...
                <select
                  value={inst.status}
                  onChange={(e) =>
                    updateInstrumentStatus(index, e.target.value)
                  }
      ...
      <div className="bg-white rounded-lg p-6 max-w-lg w-full">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <textarea
              placeholder="Add summary notes for the doctor..."
              value={notes}
