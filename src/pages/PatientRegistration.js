...
function PatientRegistration() {
    ...
    const completePayment = () => {
        if (registrationData.patientInfo &&
    ...
    if (isCompleted && generatedIds) {
        return (<Layout title="Registration Complete">
        <div className="max-w-2xl mx-auto">
          <RegistrationSuccess patientId={generatedIds.patientId} appointmentId={generatedIds.appointmentId} registrationData={registrationData} onBackToDashboard={handleBackToDashboard}/>
        </div>
      </Layout>);
    }
    if (showPaymentStep && !isCompleted) {
        return (<Layout title="Payment Processing">
        <div className="max-w-4xl mx-auto">
          <PaymentMethodSelection data={registrationData} updateData={updateRegistrationData} onComplete={completePayment} onBack={() => setShowPaymentStep(false)}/>
        </div>
      </Layout>);
    }
    ...
    return (<Layout title="">
      <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:flex md:items-center md:justify-between gap-4">
            {steps.map((step, index) => (<div key={step.number} className="flex items-center">
                ...
                : "text-gray-400"}`}>
                  ...
                  <p className={`text-sm ${currentStep >= step.number
                ? "text-gray-900"
                : "text-gray-500"}`}>
                    {step.title}
                  </p>
                </div>
                {index < steps.length - 1 && (<div className={`hidden md:block flex-1 h-0.5 mx-4 ${currentStep > step.number ? "bg-blue-600" : "bg-gray-300"}`}/>)}
              </div>))}

            {/* Payment Step Indicator */}
            <div className="flex items-center">
              <div className={`hidden md:block flex-1 h-0.5 mx-4 ${showPaymentStep || isCompleted ? "bg-blue-600" : "bg-gray-300"}`}/>
              <div className={`flex items-center justify-center w-10 h-10 rounded-full border-2 ${showPaymentStep || isCompleted
            ? "bg-blue-600 border-blue-600 text-white"
            : "border-gray-300 text-gray-400"}`}>
                5
              </div>
              <div className="ml-3">
                <p className={`text-sm font-medium ${showPaymentStep || isCompleted
            ? "text-blue-600"
            : "text-gray-400"}`}>
                  Step 5
                </p>
                <p className={`text-sm ${showPaymentStep || isCompleted
            ? "text-gray-900"
            : "text-gray-500"}`}>
                  Payment Processing
                </p>
        ...
        <div className="bg-white rounded-lg shadow-sm p-8">
          <CurrentStepComponent data={registrationData} updateData={updateRegistrationData} onNext={nextStep} onPrev={prevStep} onComplete={completeRegistration} currentStep={currentStep}/>
        </div>
      </div>
    </Layout>);
}
